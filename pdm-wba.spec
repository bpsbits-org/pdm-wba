...
# 2. Directories → 755
find "${WBA_DIR}/" -type d -exec sh -c 'install -dm755 "$1" "${BLD_DIR}/${1#./}"' _ {} \;
# 3. Shell scripts → 755
find "${WBA_DIR}/" -name "*.sh" -type f -exec sh -c 'install -Dm755 "$1" "${BLD_DIR}/${1#./}"' _ {} \;
# 4. All other files → 644
find "${WBA_DIR}/" -type f ! -name "*.sh" -exec sh -c 'install -Dm644 "$1" "${BLD_DIR}/${1#./}"' _ {} \;
